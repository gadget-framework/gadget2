...
The most recent version of Gadget will be available as a gzipped tarfile from this directory.  This file will be called gadget$<$version$>$.tar.gz, where $<$version$>$ represents the version number of Gadget.  As a guide, the current version of Gadget is 2.1.04, so the file to be downloaded is gadget2.1.04.tar.gz.
...
Throughout this document any text in $<$angled brackets$>$ should be adjusted by the user to make it relevant to the current situation, so for example the text above should be altered to the following to compile Gadget version 2.1.04 on a Linux machine:

{\small\begin{verbatim}
gunzip gadget2.1.04.tar.gz
tar -xvf gadget2.1.04.tar
cd source
make
\end{verbatim}}
...
\psi_{L} = \frac{\displaystyle \sum_{\it preys} F_{p}(l, L)}{H A \Delta t + \displaystyle \sum_{\it preys} F_{p}(l, L)}
\end{equation}
...
For fleets of type LinearFleet or EffortFleet, the data file is a list of year, timestep, area, fleetname and then the scaling factor to be used when calculating the amount of the catch for that timestep/area/fleet combination:
...
\item added a fleet that uses catch in nu\begin{enumerate}
\item defining a simple switch (called ''age2'') without an initial value
{\small\begin{verbatim}
#age2
\end{verbatim}}
...
\end{verbatim}}
\end{enumerate}
mbers data (see section~\ref{sec:numberfleet})
