...
\sigma_{ij} =
\begin{cases}
  \displaystyle \sum^{lag}_{l=1} c_l \sigma_{i-l,j} + \delta^i_{j} \sigma^2
  & \textrm{if $i \geq j$}\\
  \displaystyle \sum^{lag}_{l=1} c_l \sigma_{j,i-l}
  & \textrm{otherwise}
\end{cases}
\end{equation}
...
The $<$stocknames$>$ vector contains a list of all the fleets to be aggregated into a single pseudo fleet for the purposes of the data comparison.  The $<$suitability parameters$>$ define the suitability of the survey fleet that was used to collect the survey index data.  This is the same format as the suitability functions for the stock, as discussed in section~\ref{subsec:suitability} above.  Note that only one set of suitability values is defined, which will be applied to all the stocks for this likelihood component.
...
The Broyden-Fletcher-Shanno-Golberg, or simply BFGS, is a quasi-Newton
method and as such it accumulates information from successive line
minimizations so that $N$ such line minimizations lead to the exact
mininum of a quadratic form in $N$ dimensions. Also the method will be
quadratically convergent for general smooth functions. \newline


The basic idea of the BFGS is to build up, iteratively, a good
approximation to the inverse Hessian matrix $A^{-1}$, that is to
construct a sequence of matrices $H_i$ with the property, 
$$ lim_{i->\infty} H_i = A^{-1} $$
Even better if the limit is achieved after $N$ iterations instead of
$\infty$. This approximation is the used as follows: \newline
The minimum point $x_m$ satisfies $$Ax_m = b$$ At the current point
$x_i$, whatever it is, we have $$Ax_i = \nabla f (x_i) + b$$
Subtracting these two equations and multiplying by the inverse matrix
$A^{-1}$ we have $$x_m - x_i = A^{-1}[-\nabla f(x_i) ]$$ which leads
to $$x_{i+1} - x_i = A^{-1} (\nabla f_{i+1} - \nabla f_i)$$ where
$\nabla f_j = \nabla f(x_j)$. We might then imagine that the update
should be on the form $$H_{i+1} = H_i + \textup{correction}$$ Here the
update formula is:
\begin{equation}\label{eq:bfgsup}
H_{i+1} = H_i + \frac{(x_{i+1} - x_i)(x_{i+1} - x_i)'}{(x_{i+1} -
  x_i)'(\nabla f_{i+1} - \nabla f_i)} - \frac{[H_i (\nabla f_{i+1} -
  \nabla f_i)][H_i (\nabla f_{i+1} - \nabla f_i)]'}{(\nabla f_{i+1} -
  \nabla f_i)'H_i (\nabla f_{i+1} - \nabla f_i)} + [(\nabla f_{i+1} -
  \nabla f_i) H_i (\nabla f_{i+1} - \nabla f_i)] u u'
\end{equation}
where $$u = \frac{(x_{i+1} - x_i)}{(x_{i+1} - x_i)'(\nabla f_{i+1} -
  \nabla f_i)} - \frac{[H_i (\nabla f_{i+1} - \nabla f_i)]}{(\nabla f_{i+1} -
  \nabla f_i)'H_i (\nabla f_{i+1} - \nabla f_i)}
$$

Now to find the next point $x_{i+1}$ after the given point $x_i$ a
linesearch is conducted along $$s = - H_i \nabla f_i$$ and our new
value $x_{i+1}$ is the point accepted by the linesearch on the line
$x_i + \alpha s$. This is then continued until either the maximum
number of iterations is reached or $||\nabla f_i ||$ falls below a
certain number. \newline

The linesearch algorithm used in this implementation is the Armijo
linesearch. We start with the trial stepsize $st$ and continue
with $\beta st, \beta^2 st, \beta^3 st, ..., $ until the
first time that $\beta^m st$ falls within the set of stepsizes
$\alpha$ satisfying the inequality
\begin{equation}\label{eq:armijo}
  f(x_i) - f(x_i + \alpha s) \geq - \sigma \alpha \nabla f(x_i) ' s
\end{equation}
...
bfgseps    1e-03 ; minimum epsilon, bfgs halt criteria
beta       0.01  ; linesearch convergence parameter
sigma      0.5   ; linesearch convergence parameter
st         1.0   ; linesearch convergence parameter
\end{verbatim}}
...
bfgsiter   10000 ; number of bfgs iterations
bfgseps    1e-03 ; minimum epsilon, bfgs halt criteria
rho        0.01  ; linesearch convergence parameter
tau        0.5   ; linesearch convergence parameter
\end{verbatim}}

It should be noted that the optimisation algorithms will always be performed in the following order, irrespective of the order that they are specified in the input file: first Simulated Annealing, second Hooke and Jeeves and finally BFGS.

\subsection{Parameters}\label{subsec:combinepar}
The parameters for this combined optimisation algorithm are the same as for the individual algorithms, and are described in sections \ref{subsec:hookepar} (for the Hooke and Jeeves parameters), \ref{subsec:simannpar} (for the Simulated Annealing parameters) and \ref{subsec:bfgspar} (for the BFGS parameters)
