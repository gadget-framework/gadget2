...
from equation \ref{eq:catchdist6} below:

\begin{equation}\label{eq:catchdist6}
\ell = \frac{1}{2\sigma^2} \sum_{\it time}((L-1) log(\sigma)+\sum_{\it areas}\sum_{\it  ages}\sum_{\it lengths} \nu_{tral}^2 )
\end{equation}
where: $$\nu_{lt}=log(P_{tral})-log(\pi_{tral}})-\frac{1}{L}\sum_{\it length}
(log(P_{tral})-log(\pi_{tral}))$$
$L$ is the number of length groups\newline
$<$ P $>$ is the proportion of the data sample for that time/area/age combination\newline
$<\pi>$ is the proportion of the model sample for that time/area/age
  combination

\noindent To specify this likelihood function it is necessary to
specify the parameter $<$sigma$>$. This is done in the likelihood
file as shown below:
{\small\begin{verbatim}
...
function             mvlogistic
sigma                <sigma>   
aggregationlevel     <0 or 1> ; 1 to aggregate data over the whole
year
...
\end{verbatim}}
