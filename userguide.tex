...
\subsection{Length Selection}\label{subsec:lselection}
...
\subsubsection{Constant Selection Function}
...
\subsubsection{StraightLine Selection Function}
...
\subsubsection{Exponential Selection Function}
...
Note: when the value of the parameter is exactly equal to the bound, this equation will give a zero likelihood score
...
\sigma_{ij} =
\begin{cases}
  \sum^{lag}_{l=1} c_l \sigma_{i-l,j} + \delta^i_{j} \sigma^2
  & \textrm{if $i \geq j$}\\
  \sum^{lag}_{l=1} c_l \sigma_{j,i-l}
  & \textrm{otherwise}
\end{cases}
\end{equation}
...
\ell = \frac{1}{2\sigma^2} \sum_{\it time} \Big((L - 1) log(\sigma)+\sum_{\it areas}\sum_{\it ages}\sum_{\it lengths} \nu_{tral}^2 \Big)
\end{equation}

\begin{equation}\label{eq:catchdist6b}
\nu_{tral} = log(P_{tral}) - log(\pi_{tral}) - \frac{1}{L}\sum_{\it lengths} \Big(log(P_{tral}) - log(\pi_{tral}) \Big)
\end{equation}
...
{\small\begin{verbatim}
hooke
hookeiter  1000  ; number of hooke and jeeves iterations (default 1000)
hookeeps   1e-04 ; minimum epsilon, hooke halt criteria (default 1e-04)
...
\subsection{File Format}
To specify the Simulated Annealing optimisation algorithm, the optimisation file should start with the keyword ''simann'', followed by (up to) 9 lines giving the parameters for the optimisation algorithm.  Any parameters that are not given in the file are given default values, which work reasonably well for simple Gadget models.  The format for this file, and the default values for the optimisation algorithm, is shown below:
