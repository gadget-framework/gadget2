...
\bigskip
The ability to run in parallel across a network of computers using PVM was implemented primarily by Au{\dh}bj\"{o}rg Jakobsd\'{o}ttir with input from several other people, including J\'{o}n Sigur{\dh}sson who has tested and implemented Condor-PVM.  Several mathematical additions, e.g. for parallel minimisation on a network were implemented by {\TH}\'{o}rd\'{i}s Linda {\TH}\'{o}rarinsd\'{o}ttir and Kristjana \'{Y}r J\'{o}nsd\'{o}ttir.  Recent additions include major cleanups, modifications of input formats and debugging operations by James Begley who is the current maintainer of the code and coordinator of all programming efforts.
...
The most recent version of Gadget will be available as a gzipped tarfile from this directory.  This file will be called gadget$<$version$>$.tar.gz, where $<$version$>$ represents the version number of Gadget.  As a guide, the current version of Gadget is 2.0.03, so the file to be downloaded is gadget2.0.03.tar.gz.
...
Throughout this document any text in $<$angled brackets$>$ should be adjusted by the user to make it relevant to the current situation, so for example the text above should be altered to the following to compile Gadget version 2.0.03 on a Linux machine:

\begin{verbatim}
gunzip gadget2.0.03.tar.gz
tar -xvf gadget2.0.03.tar
cd source
make
\end{verbatim}
...
The $<$growth function$>$ defines what growth function is to be used to calculate the growth of the stock.  Currently, there is are 7 growth functions defined, so valid functions are:
...
This growth function is an form of the Jones growth equation, extended to introduce the concept of starvation to the growth function.  The increase in the weight for each length group the stock is given by equation~\ref{eq:growth4w}, and the corresponding increase in the length of the stock is given by equation~\ref{eq:growth4lc} below:
...
\bigskip
In equation~\ref{eq:maxcons}, the parameter $<$M$>$ gives the maximum possible consumption for the predator.  This is a function of the length of the predator, and 4 parameters specified by the $<$maximum consumption$>$ vector in the input file.
...
This maturity function is very similar to AgeAndLength maturity function, with the maturity proportion being calculated according to the maturity equation~\ref{eq:mat4} given below:
...
Note that since this maturation function acts on all timesteps, the $<$maturitystep$>$ vector is not specified.
...
\bigskip
The $<$minimum probability$>$ value is the default value used whenever the calculated probability is very unlikely.  This means that the likelihood component is not dominated by one or two stray values, since these will be reset back to less unlikely values.
...
\bigskip
The $<$function name$>$ defines what likelihood function is to be used to compare the modelled age-length catch distribution to the input age-length catch distribution.  Currently, there are 3 likelihood functions defined, and the valid function names are:
...
\ell = 2 \sum_{\it time}\sum_{\it areas}\sum_{\it ages} \Bigg( \log N_{tra}! - \sum_{\it lengths} \log N_{tral}! + \sum_{\it lengths} \Big( N_{tral} \log {\frac{\pi_{tral}}{\sum \pi_{tral}}} \Big)\Bigg)
\end{equation}
...
\ell = \sum_{\it time}\sum_{\it areas}\sum_{\it ages}\sum_{\it lengths}\Big( {\frac{ ( \pi_{tral} - N_{tral} ) ^2} {\pi_{tral} + \epsilon}} \Big)
\end{equation}
...
\ell = \sum_{\it time}\sum_{\it areas}\sum_{\it ages}\sum_{\it lengths} \Big( {\frac{ N_{tral}} { (\pi_{tral} + \epsilon )} + \log ({\pi_{tral} + \epsilon}}) \Big)
\end{equation}
...
\bigskip
lengthcalcvar - Weighted sum of squares of mean length\newline
lengthgivenvar - Weighted sum of squares of mean length with given standard deviation\newline
weightgivenvar - Weighted sum of squares of mean weight with given standard deviation\newline
weightnovar - Unweighted sum of squares of mean weight\newline
lengthnovar - Unweighted sum of squares of mean length
...
\bigskip
The $<$function name$>$ defines what likelihood function is to be used to compare the modelled age-length stock distribution to the input age-length stock distribution.  Currently, there is only 1 likelihood function defined, so the only valid function is:
...
\ell = 2 \sum_{\it time}\sum_{\it areas}\sum_{\it ages}\sum_{\it lengths} \Bigg( \log N_{tral}! - \sum_{\it stocks} \log N_{trals}! + \sum_{\it stocks} \Big( N_{trals} \log {\frac{\pi_{tral}}{\sum \pi_{trals}}} \Big)\Bigg)
\end{equation}
...
\bigskip
The use of 2 power coefficients gives increased flexibility for the likelihood component.  In general, a higher value of $<$p1$>$ applies more penalty to ''many small negative values'', where as a higher value of $<$p0$>$ applies more penalty to ''few large negative values''.  For a simple sum of squares of the migration matrix values, $<$p0$>$ should be set to 2, and $<$p1$>$ should be set to 1.
...
\bigskip
The value of $<$epsilon$>$ is used in the likelihood function to avoid problems that would arise from taking the logarithm of zero.  Epsilon is added to both the modelled and observed landings data, to ensure that these values are always positive, and thus should be set to a small number.
...
\ell = \sum_{\it time}\sum_{\it areas}\sum_{\it fleets} (\log(\pi_{trf} + \epsilon) - \log(N_{trf} + \epsilon))^2
\end{equation}

where:\newline
$<\pi>$ is the modelled catch data for that time/area/fleet combination\newline
...
which will set Gadget to write this line every 10th iteration.
...
which will set Gadget to write this line every 100th iteration.
