...
The most recent version of Gadget will be available as a gzipped tarfile from this directory.  This file will be called gadget$<$version$>$.tar.gz, where $<$version$>$ represents the version number of Gadget.  As a guide, the current version of Gadget is 2.1.05, so the file to be downloaded is gadget2.1.05.tar.gz.
...
\bigskip
Throughout this document any text in $<$angled brackets$>$ should be adjusted by the user to make it relevant to the current situation, so for example the text above should be altered to the following to compile Gadget version 2.1.05 on a Linux machine:

{\small\begin{verbatim}
gunzip gadget2.1.05.tar.gz
tar -xvf gadget2.1.05.tar
cd source
make
\end{verbatim}}
