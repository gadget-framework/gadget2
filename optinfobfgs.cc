...
int OptInfoBfgs::gaussian(double mult){
  ...
  for(i = 0; i < numvar; i++){
    ...
    for(j = i + 1; j < numvar; j++)
      if(ABS(Atemp[i][j]) > ABS(Atemp[i][maxrow]))
	maxrow = j;
...
double OptInfoBfgs::linesearch(){
  ...
  for(;;){
    ...
    if(mu < b + d)
      ...
      bnytt = b + d * cubmin(fia, fib, fima*d, fimb*d, 2, MIN(9, (mu - b)/d));
    a = b;
    b = bnytt;
    fia = fib;
    fima = fimb;
  ...
  while(stop != 0){
    ...
    fialpha = fa;
    if(d < MAX(ABS(a), ABS(b))*rathersmall)
      break;
